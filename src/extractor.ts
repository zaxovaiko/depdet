...
const PATTERNS: readonly Pattern[] = [
  ...
  { re: /use\s+([A-Za-z_$][\w$]*(?:\.[A-Za-z_$][\w$]*)*(?:\(\))?)\s+instead/i, source: 'heuristic' },
  { re: /replaced?\s+(?:by|with)\s+([A-Za-z_$][\w$]*(?:\.[A-Za-z_$][\w$]*)*(?:\(\))?)/i, source: 'heuristic' },
  { re: /prefer\s+([A-Za-z_$][\w$]*(?:\.[A-Za-z_$][\w$]*)*(?:\(\))?)/i, source: 'heuristic' },
]
...
const tryPattern = (text: string, { re, source }: Pattern): Hit | null => {
  ...
  return match && match[1] ? { replacement: match[1].trim(), source } : null
}
...
export const extract = (rawText: string): ExtractResult => {
  ...
  const hit = PATTERNS
    .map((p) => tryPattern(text, p))
    .find((r): r is Hit => r !== null)
